...
router.post('/status',  validator.body(statusValidation), function (req, res) {
    ...
    Status.addNewStatus(newStatus, (err, status) => {
        if (err) {
            res.json({ success: false, msg: 'Failed to add status', err: err });
        } else {
            res.json({ success: true, msg: 'status added', status: status });
        }
    })
});
...
router.post('/type',  validator.body(typeValidation), function (req, res) {
    ...
    Type.addNewTypeItem(newType, (err, type) => {
        if (err) {
            res.json({ success: false, msg: 'Failed to add type', err: err });
        } else {
            res.json({ success: true, msg: 'type added', type: type });
        }
    })
});
